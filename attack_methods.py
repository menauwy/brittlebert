"""
from Eric-Wallace/universal-triggers at github
...
def hotflip_attack_remove_unused(averaged_grad, embedding_matrix, trigger_token_ids,
    ...
    If increase_loss=False, then the attack reverses the sign of the gradient and tries to increase
    the loss (decrease the model's probability of the true class). For targeted attacks, you want
    to decrease the loss of the target class (increase_loss=False).
    """
    averaged_grad = averaged_grad.cpu()
    ...
    # remove all functional tokens including [unused*] from 30522 vocabulary. index:0-998
    ...
    if num_candidates > 1:  # get top k options
        ...
        # best_k_ids shape:(1,5,num_candidates)
...
def nearest_neighbor_grad(averaged_grad, embedding_matrix, trigger_token_ids,
    ...
    for token_pos, trigger_token_id in enumerate(trigger_token_ids):
        ...
        _, neighbors = tree.query(
            [stepped_trigger_token_embed], k=num_candidates)
