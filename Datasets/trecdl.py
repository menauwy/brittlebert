...
def get_queries(queries_f: Path) -> Dict[str, str]:
    """queries.tsv only include 200 test queries
    queries = {}
    ...
    queries = {}
    with open(queries_f, 'r')as f:
        for q_id, p_id, q, p in csv.reader(f, delimiter='\t'):
            queries[q_id] = q
    return queries
...
class TRECDL2019Document(ParsableDataset):
        ...
        def _read_queries(fname):
            ...
            with open(fname, encoding='utf-8', newline='') as fp:
                for q_id, query in csv.reader(fp, delimiter='\t'):
                    result[q_id] = query
            return result
        train_queries = _read_queries(
            self.directory / 'msmarco-doctrain-queries.tsv')
        dev_queries = _read_queries(
            self.directory / 'msmarco-docdev-queries.tsv')
        test_queries = _read_queries(
    ...
    def get_qrels(self) -> Dict[str, Dict[str, int]]:
        ...
        """
        qrels = {}
        qrels.update(read_qrels_trec(
            self.directory / 'msmarco-doctrain-qrels.tsv'))
        qrels.update(read_qrels_trec(
            self.directory / 'msmarco-docdev-qrels.tsv'))
    ...
    def get_pools(self) -> Dict[str, Set[str]]:
        ...
        top.update(read_top_trec(self.directory /
                   'msmarco-doctest2019-top100'))
        return top
