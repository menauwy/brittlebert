...
from Datasets import trec, trecdl
...
def read_top_docs(q_id: str, documents_file: Path, top_file: Path) -> List[str]:
    ...
    #['80', 'Q0', 'clueweb09-en0008-49-09144', '1', '-5.66498569', 'indri']
...
class InferenceDatasetBase(Dataset, abc.ABC):
    ...
    def __init_q_docs__(self, q_id: str, query: str):
        ...
        if self.DATA == 'clueweb09':
            self.top_docs_id, self.top_docs = trec.read_top_docs(
                q_id, self.documents_file, self.top_file)
        elif self.DATA == 'msmarco_p':
            self.top_docs_id, self.top_docs = trecdl.read_top_docs(
                q_id, self.documents_file, self.top_file)
...
"""
class BatchDataset(Dataset, abc.ABC):
    # Abstract base class for inference data,
    # e.g, multiple queries and multiple relavent top 1 documents

    def __init__(self, documents_file: Path, top_file: Path, batch_ids: dict):
        #self.query = query
        #self.top_docs = read_top_docs(q_id, documents_file, top_file)
        self.length = []
        self.documents_file = documents_file
    ...
    def __init_q_docs__(self, q_ids: list, queries: list):
        self.query = queries
        self.top_docs_id = []
        self.top_docs = []
        self.q_ids = q_ids
        ...
        for index, q_id in enumerate(q_ids):
            i, j = read_top_docs(q_id, self.documents_file, self.top_file)
            self.top_docs_id.append(i)
            self.top_docs.append(j)
            self.length.append(len(j))
            doc_index = self.batch_ids[index][1]
            self.batch_docs.append(self.top_docs[index][doc_index])
    ...
    def __getitem__(self, index: int) -> Input:
        # get batch of (doc and query) pairs
        # (query with its corresponding NO.1 doc)
        doc = self.batch_docs[index]
        return self.get_single_input(self.query[index], doc)

    def __buildFromDoc__(self, doc: list) -> Input:
        return self.get_single_input(self.query, doc)
"""
